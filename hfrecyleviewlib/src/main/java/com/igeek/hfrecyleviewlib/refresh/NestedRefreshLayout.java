 ...
 * description :  具有下拉刷新功能
 * <p>
 * The following methods you need to know:
 * <ul>
 * <li>{@link #setRefreshing(boolean)}</li>
 * <li>{@link #isRefreshing()}</li>
 * </ul>
 * </p>
 */
public class NestedRefreshLayout extends ViewGroup implements NestedScrollingParent, NestedScrollingChild {
    ...
    public NestedRefreshLayout(Context context, AttributeSet attrs) {
        ...
        mTouchSlop = ViewConfiguration.get(context).getScaledTouchSlop(); //最小移动距离
...
//        Log.e(TAG, "onInterceptTouchEvent: ");
        ...
        if ((action == MotionEvent.ACTION_MOVE) && (mIsBeingDragged) && !isForbid) {
            return true;
        }
...
//                "\nrefreshHeight="+pullHelper.getPullRefreshHeight()+"\npullMaxHegiht="+pullHelper.getMaxHeight());
    ...
    /**
     * false close otherwise open
     *
     * @param refreshing
     */
    public void setRefreshing(boolean refreshing) {
        if (refreshing && !isForbid)
            animToPosition(0, -pullHelper.getPullRefreshHeight(), 0, refreshingListener);
        else
            refreshDelayFinish(800);
    }
    ...
    public void animToPosition(int from, int to, long delayTime, Animation.AnimationListener listener) {
        animation.reset();
...
//        final boolean canFling = canFilngUp || canFilngDown;
     ...
     * @param forbid
     */
    public void setForbidRefresh(boolean forbid) {
